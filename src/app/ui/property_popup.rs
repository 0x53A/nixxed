...
impl App {
    ...
    fn draw_property_description(&self, frame: &mut Frame, area: Rect) {
        let description = if self.prop_editor.showing_available {
            ...
            self.prop_editor.list_state.selected()
                        ...
                        .map(|prop| {
                            ...
                            format!("{} = {} ({})", prop.name, prop.value, 
                                match prop.property_type {
                                    ...
                                    PropertyType::AttrSet => if prop.name.contains('.') { 
                                        "nested attribute" 
                                    } else { 
                                        "attribute set" 
                                    },
    ...
    fn draw_configured_properties(&mut self, frame: &mut Frame, area: Rect) {
        ...
        let items: Vec<ListItem> = if properties.is_empty() {
            vec![ListItem::new(Line::from(vec![
                Span::styled("  (no properties defined - press Tab to see available)", 
                    Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC)),
            ]))]
        } else {
            properties.iter().enumerate().map(|(idx, prop)| {
                // Determine type indicator - prefer schema info if we have it
                let (type_indicator, type_label) = self.get_property_type_info(&prop.name, &prop.property_type);
                
                let is_selected = self.prop_editor.list_state.selected() == Some(idx);
                let is_editing = self.prop_editor.edit_state.as_ref()
                ...
                ListItem::new(Line::from(vec![
                    Span::styled(format!("{} ", type_indicator), Style::default().fg(Color::Cyan)),
        ...
        if item_count > visible_height {
            ...
            let (content_len, position, use_decorators, viewport_for_thumb) = calculate_scrollbar_position(
                viewport_start, item_count, visible_height
            );
    ...
    pub(crate) fn get_property_type_info(&self, prop_name: &str, fallback_type: &PropertyType) -> (&'static str, String) {
        ...
        if let Some((_, info)) = self.prop_editor.available_options.iter().find(|(n, _)| n == prop_name) {
            return (
        ...
        if prop_name.contains('.') || prop_name.contains('"') {
            ...
            if let Some((_, info)) = self.prop_editor.available_options.iter().find(|(n, _)| n == base) {
                return (
                    "🔧",
                    format!("nested in {}", info.option_type)
                );
            }
    ...
    fn draw_available_options(&mut self, frame: &mut Frame, area: Rect) {
        // Add title block
        let title = format!(" Available ({}) - Tab for configured ", self.prop_editor.available_options.len());
        let block = Block::default()
            .borders(Borders::ALL)
        ...
        let items: Vec<ListItem> = if self.prop_editor.available_options.is_empty() {
            vec![ListItem::new(Line::from(vec![
                Span::styled("  (no available options found - schema may not be loaded)", 
                    Style::default().fg(Color::DarkGray).add_modifier(Modifier::ITALIC)),
            ]))]
        } else {
            self.prop_editor.available_options.iter().enumerate().map(|(idx, (opt_name, opt_info))| {
                let type_indicator = type_indicator_for_nix_type(&opt_info.option_type);
                
                let is_selected = self.prop_editor.list_state.selected() == Some(idx);
                
                // Get default value for display
                let default_str = opt_info.default.as_ref()
                    .map(|v| match v {
                        serde_json::Value::Bool(b) => b.to_string(),
                        serde_json::Value::Number(n) => n.to_string(),
                        serde_json::Value::String(s) => {
                            if s.len() > 15 { format!("\"{}...\"", &s[..12]) } else { format!("\"{}\"", s) }
                        },
                ...
                ListItem::new(Line::from(vec![
                    Span::styled(format!("{} ", type_indicator), Style::default().fg(Color::Blue)),
                    Span::styled(opt_name.clone(), style.add_modifier(Modifier::BOLD)),
                    Span::styled(format!(" [{}]", type_display), Style::default().fg(Color::DarkGray)),
                    Span::styled(format!(" = {}", default_str), Style::default().fg(Color::Cyan)),
                ]))
            }).collect()
        };
